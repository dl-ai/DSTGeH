...
logging.basicConfig(level=logging.INFO,filename='euc_similar_matrix_imagenet_cifar10.log',format="%(levelname)s:%(asctime)s:%(message)s")

def euclidean_distance(vector_a, vector_b):
    eucdist = np.linalg.norm(vector_a - vector_b)
    return eucdist

def euc_similar_matrix(feature_set, feature_num):
    relations = np.zeros((feature_num, feature_num))
    for i in range(0,feature_num):
        for j in range(i,feature_num):
            eucdist = euclidean_distance(feature_set[i], feature_set[j])
            if i < j:
                relations[i][j] = eucdist
                relations[j][i] = eucdist
            elif i == j:
                relations[i][j] = 0
    return relations
...
if __name__ == "__main__":
    ...
    h5file_relations = h5py.File(os.path.join(__PATH__GRAPH, 'euc_relations_cifar10_imagenet.h5'), 'w')
    ...
    relations = euc_similar_matrix(feature_set, feature_num)
    ...
    h5file_relations.create_dataset("euc_relations_cifar10_imagenet", data=relations)
