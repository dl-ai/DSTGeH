...
logging.basicConfig(level=logging.INFO, filename='resnet18_MMloss_48.log', format="%(levelname)s:%(asctime)s:%(message)s")
...
if torch.cuda.is_available():
    model.cuda()
loss_fc = nn.MultiLabelMarginLoss()
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        ...
        logging.info(epoch)
        print(loss)
        torch.save(model.state_dict(), '../data/cifar10/hashmodel/whole/48/cifar10_whole_hashmodel_48_MM.pth')
